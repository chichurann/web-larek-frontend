...
Слой Presenter не выносится в отдельные классы и будет реализован в файле index.ts.
...
##Слой Презентации
Этот слой отвечает за визуализацию данных и взаимодействие пользователя с интерфейсом. Сюда относятся только интерфейсы и типы, которые влияют на представление и пользовательский опыт.

Интерфейсы и Типы для Презентации
Здесь интерфейсов нет, так как они описывают сущности данных и их состояния. Этот слой работает с теми данными, которые уже типизированы в слоях Данных и Представления.


##События, возникающие при взаимодействии пользователя с интерфейсом

1. catalog:updated — обновление списка товаров
При обновлении каталога товаров создается новый список карточек, отображаемый на странице. Каждая карточка связывается с событием card:openDetails для открытия её подробной информации.

2. card:openDetails — открытие детальной карточки товара
Событие срабатывает при нажатии на карточку товара:
- Открывается модальное окно с детальной информацией.
- Пользователь может добавить товар в корзину, инициируя событие basket:addProduct.

3. basket:addProduct — добавление товара в корзину
- Продукт добавляется в корзину.
- Состояние корзины обновляется.
- Модальное окно корзины автоматически открывается.

4. basket:open — открытие корзины
Вызывает показ модального окна с текущим содержимым корзины.

5. basket:changed — обновление состояния корзины
- Пересчитывается количество товаров в корзине.
- Обновляется итоговая сумма.
- Отображается обновленный список товаров.

6. order:openForm — открытие формы заказа
Открывает форму ввода данных для оформления заказа (например, адреса доставки).

7. order:formUpdated — изменение данных формы заказа
- Срабатывает при обновлении любых данных в форме.
- Проверяет валидность формы.
- Отражает изменения в визуальном состоянии (например, блокирует кнопку "Подтвердить", если форма невалидна).

8. order.address:change — обновление адреса доставки
Изменяет поле адреса в состоянии заказа.

9. formErrors:changed — обновление ошибок формы
- Проверяет валидность введенных данных.
- Отображает ошибки для пользователя.

10. /contacts\.(phone|email):change/ — изменение контактов
Обновляет поля телефона и email в состоянии заказа.

11. order:submit — подтверждение заказа
- Завершает процесс оформления заказа.
- Переход к вводу контактных данных.

12. contacts:submit — отправка формы контактов
Завершает заказ, выполняя следующие действия:
- Отправляет данные на сервер.
- Очищает корзину и состояние заказа.
- Показывает сообщение о успешном оформлении.

13. modal:open — открытие модального окна
Блокирует прокрутку страницы, чтобы сосредоточить внимание пользователя.

14. modal:close — закрытие модального окна
